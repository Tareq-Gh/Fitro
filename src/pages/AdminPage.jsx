...
export function AdminPage({ token }) {
  ...
  const stats = [
    ...
    {
      label: t("admin.avgHeight"),
      value: avgHeight ? `${avgHeight} cm` : "—",
      icon: Ruler,
      color: "text-purple-400",
    },
    {
      label: t("admin.measurements"),
      value: users.length,
      icon: ShoppingBag,
    ...
    <div className="w-full max-w-6xl mx-auto p-4 md:p-8 mt-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {stats.map((stat, i) => (
          <div
            key={i}
            className="bg-white/5 backdrop-blur-md p-6 rounded-[30px] border border-white/10 hover:border-cyan-500/50 hover:-translate-y-0.5 transition-all"

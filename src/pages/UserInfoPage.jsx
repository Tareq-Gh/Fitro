...
const FIT_COLORS = {
  Tight: { badge: "bg-red-100 text-red-700", icon: AlertTriangle, iconColor: "text-red-500" },
  "Slightly Tight": { badge: "bg-orange-100 text-orange-700", icon: AlertTriangle, iconColor: "text-orange-400" },
  "Perfect Fit": { badge: "bg-green-100 text-green-700", icon: CheckCircle2, iconColor: "text-green-500" },
  Comfortable: { badge: "bg-cyan-100 text-cyan-700", icon: CheckCircle2, iconColor: "text-cyan-500" },
  Loose: { badge: "bg-blue-100 text-blue-700", icon: Info, iconColor: "text-blue-400" },
  Oversized: { badge: "bg-purple-100 text-purple-700", icon: Info, iconColor: "text-purple-400" },
};
...
function BtnGroup({ label, options, value, onChange }) {
  return (
    <div>
      <p className="text-xs text-gray-400 uppercase tracking-wider mb-2 font-semibold">{label}</p>
...
export function UserInfoPage() {
  ...
  // phases: email -> body (new user) OR mode (returning user) -> garment -> result
  const [phase, setPhase] = useState("email");
  const [mode, setMode] = useState(null); // "REPORT_ONLY" | "IMAGE_AND_REPORT"
  const [email, setEmail] = useState(() => localStorage.getItem("fitro_email") ?? "");
  const [welcomeBack, setWelcomeBack] = useState(null);
  const [lookupLoading, setLookupLoading] = useState(false);
  const [body, setBody] = useState(() => {
    try { return JSON.parse(localStorage.getItem("fitro_body") ?? "{}"); }
    catch { return {}; }
  });
  ...
  async function handleAnalyze(e) {
    e.preventDefault();
    if (!product.category || !product.region || !product.fit_type || !product.material) {
  ...
  if (phase === "email") {
    return (
      <div className="bg-white rounded-[45px] shadow-2xl p-8 w-full max-w-[440px] mx-4" style={{ animation: "scale-in 0.35s ease both" }}>
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-800">{t("userInfo.emailGateTitle")}</h2>
          <p className="text-gray-400 text-xs mt-1 text-center px-4">{t("userInfo.emailGateSub")}</p>
        </div>
        ...
        <button
          onClick={() => { setEmail(""); setPhase("body"); }}
    ...
    return (
      <div className="bg-white rounded-[45px] shadow-2xl p-8 w-full max-w-[440px] mx-4" style={{ animation: "scale-in 0.35s ease both" }}>
        <div className="flex flex-col items-center mb-4">
          ...
          <h2 className="text-xl font-bold text-gray-800">{t("userInfo.step1Title")}</h2>
          <p className="text-gray-400 text-xs mt-1">{t("userInfo.step1Sub")}</p>
        </div>
        <form onSubmit={(e) => { e.preventDefault(); setPhase("mode"); }} className="space-y-3">
          <Field label={t("userInfo.name")} value={body.name} onChange={(v) => setBodyField("name", v)} required />
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              <option value="" disabled>{t("userInfo.gender")}</option>
              <option value="male">{t("userInfo.male")}</option>
              <option value="female">{t("userInfo.female")}</option>
            </select>
            <Field label={t("userInfo.height")} type="number" value={body.height_cm} onChange={(v) => setBodyField("height_cm", v)} required />
          </div>
          <Field label={t("userInfo.weight")} type="number" value={body.weight_kg} onChange={(v) => setBodyField("weight_kg", v)} required />
          <Field label={t("userInfo.chest")} type="number" value={body.chest_cm} onChange={(v) => setBodyField("chest_cm", v)} />
          <Field label={t("userInfo.waist")} type="number" value={body.waist_cm} onChange={(v) => setBodyField("waist_cm", v)} />
          <Field label={t("userInfo.hips")} type="number" value={body.hips_cm} onChange={(v) => setBodyField("hips_cm", v)} />
          <button
            type="submit"
      ...
      <div className="bg-white rounded-[45px] shadow-2xl p-8 w-full max-w-[440px] mx-4" style={{ animation: "scale-in 0.35s ease both" }}>
        {welcomeBack && (
          <div className="flex items-center gap-2 bg-green-50 border border-green-100 rounded-2xl px-4 py-3 mb-5">
            ...
            <p className="text-green-700 text-xs">{t("userInfo.welcomeBack").replace("{name}", welcomeBack.name)}</p>
          </div>
        )}
        <div className="text-center mb-6">
          <h2 className="text-xl font-bold text-gray-800">{t("userInfo.modeTitle")}</h2>
          <p className="text-gray-400 text-xs mt-1">{t("userInfo.modeSub")}</p>
        </div>
        <div className="space-y-3">
          <button
            onClick={() => { setMode("REPORT_ONLY"); setPhase("garment"); }}
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="font-bold text-gray-800 text-sm">{t("userInfo.modeReport")}</span>
                  <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-medium">{t("userInfo.modeReportBadge")}</span>
                </div>
                <p className="text-gray-400 text-xs">{t("userInfo.modeReportDesc")}</p>
              </div>
            </div>
          </button>

          <button
            onClick={() => { setMode("IMAGE_AND_REPORT"); setPhase("garment"); }}
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="font-bold text-gray-800 text-sm">{t("userInfo.modeVisual")}</span>
                  <span className="text-[10px] bg-purple-50 text-purple-500 px-2 py-0.5 rounded-full font-medium">{t("userInfo.modeVisualBadge")}</span>
                </div>
                <p className="text-gray-400 text-xs">{t("userInfo.modeVisualDesc")}</p>
              </div>
            </div>
          </button>
        </div>
  ...
  if (result) {
    ...
    return (
      <div className="bg-white rounded-[45px] shadow-2xl p-8 w-full max-w-[480px] mx-4" style={{ animation: "scale-in 0.35s ease both" }}>
        <div className="text-center mb-6" style={{ animation: "fade-up 0.4s ease both" }}>
          <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 ${result.fit_result === "Perfect Fit" ? "bg-green-100" : "bg-gray-100"}`}>
            <FitIcon className={fitStyle.iconColor} size={32} />
          </div>
          <h2 className="text-2xl font-bold text-gray-800">{t("userInfo.resultTitle")}</h2>
          <p className="text-gray-400 text-sm mt-1">{categoryLabel} · {product.size_label} · {product.region}</p>
        </div>

        <div className="flex items-center justify-center gap-3 mb-6" style={{ animation: "fade-up 0.4s 0.08s ease both" }}>
          <span className={`px-5 py-2 rounded-full text-sm font-bold ${fitStyle.badge}`}>{t(`userInfo.${result.fit_result}`)}</span>
          <span className={`px-3 py-2 rounded-full text-xs font-medium ${CONFIDENCE_COLORS[result.confidence]}`}>
            {t(`userInfo.${result.confidence}`)} {t("userInfo.confidence")}
          </span>
        </div>

        <div className="bg-gray-50 rounded-2xl p-4 mb-4" style={{ animation: "fade-up 0.4s 0.15s ease both" }}>
          <p className="text-xs text-gray-400 uppercase tracking-wider mb-2 font-semibold">{t("userInfo.analysisLabel")}</p>
          <p className="text-gray-700 text-sm leading-relaxed">{result.explanation}</p>
        </div>

        <div className="border border-cyan-100 bg-cyan-50 rounded-2xl p-4 mb-4" style={{ animation: "fade-up 0.4s 0.22s ease both" }}>
          <p className="text-xs text-cyan-500 uppercase tracking-wider mb-2 font-semibold">{t("userInfo.adviceLabel")}</p>
          <p className="text-gray-700 text-sm leading-relaxed">{result.advice}</p>
        </div>

        {mode === "IMAGE_AND_REPORT" && (
          <div className="border-2 border-dashed border-purple-200 rounded-3xl p-5 mb-4" style={{ animation: "fade-up 0.4s 0.3s ease both" }}>
            <div className="flex items-center gap-2 mb-3">
              <Sparkles size={15} className="text-purple-500" />
              <span className="text-sm font-bold text-gray-800">{t("userInfo.tryOnTitle")}</span>
              <span className="text-[10px] bg-purple-50 text-purple-500 px-2 py-0.5 rounded-full font-medium">{t("userInfo.modeVisualBadge")}</span>
            </div>
            <div className="flex flex-col items-center py-3">
              ...
              <p className="text-gray-400 text-xs text-center leading-relaxed">{t("userInfo.tryOnNote")}</p>
              <div className="flex gap-2 mt-3 flex-wrap justify-center">
                <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-1 rounded-full">{product.fit_type}</span>
                <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-1 rounded-full">{product.material}</span>
                <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-1 rounded-full">{categoryLabel}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <button
          onClick={() => { setResult(null); setProduct({}); setGarment({}); setPhase("mode"); }}
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center mb-5">
        ...
        <h2 className="text-xl font-bold text-gray-800">{t("userInfo.step2Title")}</h2>
      ...
      <div className="space-y-5">
        ...
        <BtnGroup
          ...
          options={["EU", "US", "IT", "TU", "CH"].map((r) => ({ value: r, label: r }))}
        />
        <BtnGroup
        ...
        />
        <Field label={t("userInfo.sizeLabel")} value={product.size_label} onChange={(v) => setProductField("size_label", v)} />

        <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold">{t("userInfo.garmentMeasurements")}</p>
        {product.category !== "pants" && (
          <Field label={t("userInfo.garmentChest")} type="number" value={garment.chest_cm} onChange={(v) => setGarmentField("chest_cm", v)} required={!!product.category} />
        )}
        {product.category === "pants" && (
          <>
            <Field label={t("userInfo.garmentWaist")} type="number" value={garment.waist_cm} onChange={(v) => setGarmentField("waist_cm", v)} required />
            <Field label={t("userInfo.garmentHips")} type="number" value={garment.hips_cm} onChange={(v) => setGarmentField("hips_cm", v)} />
            <Field label={t("userInfo.thigh")} type="number" value={garment.thigh_cm} onChange={(v) => setGarmentField("thigh_cm", v)} />
          </>
        )}
        <Field label={t("userInfo.length")} type="number" value={garment.length_cm} onChange={(v) => setGarmentField("length_cm", v)} />
      </div>
